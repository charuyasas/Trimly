...
use App\UseCases\Booking\Requests\UpdateBookingStatusRequest;
...
class BookingController extends Controller
{
    ...
    public function update(Request $request, Booking $booking, UpdateBookingInteractor $update)
    {
        if ($request->has('status') && $request->keys() === ['status']) {
            // Status-only update
            $statusData = UpdateBookingStatusRequest::from($request);
            $booking->update(['status' => $statusData->status]);
    ...
    public function updateStatus(Booking $booking, UpdateBookingStatusRequest $request)
    {
        ...
        return response()->json(['message' => 'Booking status updated', 'status' => $booking->status]);
    }
}
