                        ...
                        <div class="tab-pane fade" id="grnStep2">
                            ...
                            <div id="discountSection" class="mt-4 border rounded p-3">
                                ...
                                <div class="d-flex align-items-center gap-2 mb-2">
                                    <input type="number" class="form-control" id="bill_discount_amount" placeholder="Discount Amount">
...
<script>
    ...
    function addItem() {
        ...
        const type = $('input[name="grn_type"]:checked').val();
        if (type === 'Profit Margin') {
            item.final_price = item.price + ((item.price * item.margin) / 100);
        } else {
        ...
        if (!item.item_id || item.qty <= 0 || item.price <= 0) {
            return Swal.fire('Error', 'Fill item, qty and price fields correctly', 'error');
        }

        if (grnItems.some(i => i.item_id === item.item_id)) {
            return Swal.fire('Duplicate', 'Item already added', 'warning');
        }
    ...
    function calculateGrnTotals() {
        let subtotal = grnItems.reduce((acc, item) => acc + item.subtotal, 0);
        let focTotal = grnItems.reduce((acc, item) => acc + (item.final_price * item.foc), 0);
        let grand = subtotal;

        if (discountConfig.amount > 0) {
            grand = discountConfig.isPercentage
                ? grand * (1 - discountConfig.amount / 100)
                : grand - discountConfig.amount;
        }
    ...
    $('#grnForm').on('submit', function (e) {
        ...
        const data = {
            ...
            grn_type: $('input[name="grn_type"]:checked').val(),
            store_location: $('#store_location').val(),
            note: $('#note').val(),
            discount_amount: discountConfig.amount,
            is_percentage: discountConfig.isPercentage,
            items: grnItems
        };

        axios.post('/api/grn', data).then(res => {
            Swal.fire('Success', 'GRN saved successfully', 'success').then(() => location.reload());
        }).catch(err => {
